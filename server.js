...
app.post('/cart', (req, res) => {
  ...
  app.delete('/cart/delete/id', (req, res) => {
    fs.readFile(CART_DATA_FILE, (err, data) => {
      ...
      if (!cartProductExists) cartProducts.splice(newCartProduct.id,1 );
      fs.writeFile(CART_DATA_FILE, JSON.stringify(cartProducts, null, 4), () => {
        res.setHeader('Cache-Control', 'no-cache');
        res.json(cartProducts);
      });
    ...
    fs.readFile(CART_DATA_FILE, (err, data) => {
      ...
      cartProducts.map((cartProduct) => {
        if (cartProduct.id === req.body.id && cartProduct.quantity > 1) {
          ...
          const cartIndexToRemove = cartProducts.find(cartProduct => cartProduct.id === req.body.id);
          cartProducts.splice(cartIndexToRemove);
        } else {
            console.log(err)
        }
      });
